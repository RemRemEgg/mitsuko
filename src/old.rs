...
fn save_pack(mut pack: Datapack) -> Datapack {
    ...
    remove_dir_all([&*pack.src_loc, "generated"].join("/")).ok();
    remove_dir_all([&*pack.src_loc, "exports"].join("/")).ok();
    let root_path = [&*pack.src_loc, "generated", &*pack.meta.name].join("/");

    remove_dir_all(&*root_path).ok();
    make_folder(&*root_path);
    make_folder(&*join![&*pack.src_loc, "/exports"]);
    ...
    for namespace in pack.namespaces.iter_mut() {
        ...
        if read_dir(&*[&*pack.src_loc, "/src/", &*namespace.id, "/extras"].join("")).is_ok() {
            copy_dir_all(&*[&*pack.src_loc, "/src/", &*namespace.id, "/extras"].join(""), &*ns_path).expect("Could not copy 'extras' folder");
        }

        let mut file =
            File::create(&*join![&*pack.src_loc,"/exports/",&*namespace.id,".msk"]).expect(&*["Could not make export file '", &*namespace.id.form_foreground(str::PNK), "'"].join(""));
